...
void Displayprefix(TrieNode* trie, int nivel, char prefix[20])
{
	int count;
	...
	if (trie)
	{
		...
		{   // display the string at his level
			...
			if (strncmp(trie->word, prefix, strlen(prefix)) == 0)
				cout << trie->word << endl;
		}
		// display all the words stored through trie children
		for (count = 26; count >= 0; count--)
			Displayprefix(trie->pChildren[count], nivel + 4, prefix);
	}
